...
def test_reliability_curve_multidim_lazy():
    ...
    f_xr = xr.DataArray(
        da.from_array(f_data, chunks=(5, 5, 50)), dims=["lat", "lon", "time"]
    )
    o_xr = xr.DataArray(
        da.from_array(o_data, chunks=(5, 5, 50)), dims=["lat", "lon", "time"]
    )

    # Compute over time dimension
    bc, of, ct = verification_metrics.compute_reliability_curve(
        f_xr, o_xr, n_bins=5, dim="time"
    )
    ...
    pixel_f = f_data[0, 0, :]
    pixel_o = o_data[0, 0, :]
    bc_exp, of_exp, ct_exp = verification_metrics.compute_reliability_curve(
        pixel_f, pixel_o, n_bins=5
    )
...
def test_brier_components_multidim_lazy():
    ...
    res = verification_metrics.compute_brier_score_components(
        f_xr, o_xr, n_bins=5, dim="time"
    )
    ...
    pixel_f = f_data[0, 0, :]
    pixel_o = o_data[0, 0, :]
    res_exp = verification_metrics.compute_brier_score_components(
        pixel_f, pixel_o, n_bins=5
    )

    np.testing.assert_allclose(
        res["reliability"].isel(lat=0, lon=0).compute(), res_exp["reliability"]
    )
    np.testing.assert_allclose(
        res["brier_score"].isel(lat=0, lon=0).compute(), res_exp["brier_score"]
    )
...
def test_reliability_diagram_plot_xarray():
    ...
    ax = plot.plot(
        ds, forecasts_col="forecast", observations_col="observation", n_bins=5
    )
