...
class SpatialBiasScatterPlot(SpatialPlot):
    ...
    def __init__(self, df: Any, col1: str, col2: str,
                 projection=ccrs.PlateCarree(), vmin: float = None,
                 vmax: float = None, ncolors: int = 15, fact: float = 1.5,
                 cmap: str = "RdBu_r", *args, **kwargs):
        """
        Initialize the plot with data and map projection.

        Args:
            df (pd.DataFrame, np.ndarray, xr.Dataset, xr.DataArray): DataFrame.
            col1 (str): Name of the first column (e.g., observations).
            col2 (str): Name of the second column (e.g., model).
            ...
            **kwargs: Keyword arguments for plotting, including cartopy features
                      like 'coastlines', 'countries', 'states', 'borders',
                      'ocean', 'land', 'rivers', 'lakes', 'gridlines'.
        """
    ...
    def plot(self, **kwargs):
        ...
        new = self.df[["latitude", "longitude", self.col1,
                       self.col2]].dropna().copy(deep=True)

        diff = new[self.col2] - new[self.col1]
        top = around(score(diff.abs(), per=95))
        c, cmap = colorbar_index(self.ncolors, self.cmap,
                                 minval=top * -1, maxval=top, ax=self.ax)
