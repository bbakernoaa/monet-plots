...
def test_imshow_robust_coords():
    ...
    da2 = xr.DataArray(np.random.rand(10, 10),
                      coords={'y': y, 'x': x},
                      dims=['y', 'x'])
...
def test_contour_xarray():
    ...
    lat = np.linspace(30, 40, 10)
    lon = np.linspace(-100, -90, 10)
    da = xr.DataArray(np.random.rand(10, 10), coords={'lat': lat, 'lon': lon}, dims=['lat', 'lon'])
...
def test_facet_grid_xarray_auto_coords():
    ...
    da = xr.DataArray(np.random.rand(2, 10, 10),
                     coords={'time': time, 'latitude': latitude, 'longitude': longitude},
                     dims=['time', 'latitude', 'longitude'], name='val')
