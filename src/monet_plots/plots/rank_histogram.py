...
class RankHistogramPlot(BasePlot):
    ...
    def plot(self,
        ...
        Args:
            data (pd.DataFrame): Data containing ranks (0 to n_members).
        ...
        """
        validate_dataframe(data, required_columns=[rank_col])
        
        if n_members is None:
            n_members = int(data[rank_col].max())
        ...
        if label_col:
            ...
            counts = data[rank_col].value_counts().reindex(np.arange(num_bins), fill_value=0)
            total = counts.sum()
            freq = counts / total if normalize else counts
