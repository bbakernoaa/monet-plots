...
def test_trajectory_lazy_xarray():
    ...
    n = 100
    lon = xr.DataArray(da.from_array(np.linspace(-120, -70, n), chunks=50), dims="time", name="lon")
    lat = xr.DataArray(da.from_array(np.linspace(25, 50, n), chunks=50), dims="time", name="lat")
    data = xr.DataArray(da.from_array(np.random.rand(n), chunks=50), dims="time", name="concentration")

    # Time series data in the same or different object
    times = pd.date_range("2023-01-01", periods=n, freq="h")
    ts_data = xr.DataArray(da.from_array(np.random.rand(n), chunks=50), coords={"time": times}, dims="time", name="obs")

    # 1. Test initialization with separate xarray objects
    plot = TrajectoryPlot(longitude=lon, latitude=lat, data=data, time=times, ts_data=ts_data)
