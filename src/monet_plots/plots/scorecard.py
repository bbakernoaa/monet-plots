...
class ScorecardPlot(BasePlot):
    ...
    def plot(self, 
        ...
        """
        validate_dataframe(data, required_columns=[x_col, y_col, val_col])
        
        # Pivot Data
        pivot_data = data.pivot(index=y_col, columns=x_col, values=val_col)
        ...
        if sig_col:
            pivot_sig = data.pivot(index=y_col, columns=x_col, values=sig_col)
